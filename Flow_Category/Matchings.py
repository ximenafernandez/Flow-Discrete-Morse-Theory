...
# This file was *autogenerated* from the file Flow_Category/Matchings.sage
from sage.all_cmdline import *   # import sage library

_sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_123 = Integer(123)
...
def is_collapsible(X):
    if is_contractible(X):
        return True
    return len(weak_core(X))==_sage_const_1 
...
def is_homotopically_trivial(X):
    ...
    H = K.homology()
    for i in H.keys():
        if H[i]!=HomologyGroup(_sage_const_0 , ZZ):
            return False
    ...
    try:
        ...
        pass
    try:
        return len(Pi1.gens()) == _sage_const_0 
    except Exception:
        pass
    try:
...
def is_regular_flow_category(P, elms_X, e):
    ...
    Q = {}
    for w in elms_X:
            if w != x and len(P[(w, y)]) != _sage_const_0 :  
                for z in elms_X:
                    if z != y and len(P[(x, z)]) != _sage_const_0 : 
...
import random
def regular_acyclic_matching2(X, p, seed_value = _sage_const_123 ):
    ...
    for e in edges:
        ...
        if D.is_directed_acyclic():
            ...
            rnd = random.uniform(_sage_const_0 , _sage_const_1 )
            print(rnd)
            if rnd<p:
...
def random_acyclic_matching(X, seed_value=_sage_const_123 ):
